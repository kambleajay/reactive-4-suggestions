...
trait WikipediaApi {
  ...
  implicit class ObservableOps[T](obs: Observable[T]) {
    ...
    def concatRecovered[S](requestMethod: T => Observable[S]): Observable[Try[S]] = {
      val res1: Observable[Observable[S]] = (obs map requestMethod)
      val res2: Observable[Observable[Try[S]]] = res1.map(xs => xs.recovered)
      val res3: Observable[Try[S]] = res2.concat
      res3
    }
